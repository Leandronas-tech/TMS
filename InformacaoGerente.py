...
def guardarDados():
   ...
   bancoGerente = sqlite3.Connection("C:/ProgramData/TMS/bancoMeus/meusDados.db")
   cursoGerente = bancoGerente.cursor()
   cursoGerente.execute("CREATE TABLE IF NOT EXISTS infoGerente(id INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL,\
                        nome TEXT, cpf TEXT, identidade INTEGER, salario REAL, funcao TEXT, entrada TEXT, dtn TEXT, email TEXT, conta TEXT)")
   inserir = "INSERT INTO infoGerente(nome, cpf, identidade, salario, funcao, entrada, dtn, email, conta) VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?)"
   for i in range(1, num+1):
      ...
      cursoGerente.execute(inserir, (listaNomes[i], listaCpfs[i], listaRgs[i], listaSalarios[i], listaFuncao[i], listaEntrada[i], listaNascimento[i], listaEmails[i], conta,))
      bancoGerente.commit()
   cursoGerente.close()
   bancoGerente.close()
def criaConta():
   ...
   for i in range(1, num+1):
      if listaConta[i] == "1":
         cursoConta1.execute(inserir1, (listaEmails[i], listaSenha[i], "Gerente", listaNomes[i],))
      elif listaConta[i] == "2":
         cursoConta2.execute(inserir2, (listaEmails[i], listaSenha[i], "Gerente", listaNomes[i],))
      else:
         cursoConta3.execute(inserir3, (listaEmails[i], listaSenha[i], "Gerente", listaNomes[i],))
      bancoConta.commit()
   cursoConta1.close()
   cursoConta2.close()
   cursoConta3.close()
   bancoConta.close() 
def gerenteJanela(f):
   global num, rep, listaNomes, listaCpfs, listaRgs, listaSalarios, listaEmails, listaNascimento, listaEntrada, listaFuncao, listaConta, listaV, listaSenha, c, corPad, corPri, corL, gerenteja, nasci, nome, cpf, rg, salario, email, entra, funcao, clickTD, btSC, btCS, btCA, variavelSC, variavelCS, variavelCA, senhaCS, senhaCA, scs, sca, buttCS, buttCA;
   c = 0
   num = f
   rep = 1
   corPad = "darkgreen"
   ...
   gerenteja = tix.Tk()
   xt = gerenteja.winfo_screenwidth()
   yt = gerenteja.winfo_screenheight()

   logo = Canvas(gerenteja, bg=corPad, height=70, relief=FLAT, highlightcolor=corPad, highlightbackground=corPad)
   logo.pack(side=TOP, expand=False, anchor=N, fill=X)

   fundo = Canvas(gerenteja, bg=corPad, height=20, relief=FLAT, highlightcolor=corPad, highlightbackground=corPad)
   ...
   ttk.Label(gerenteja, text="Nascimento:", font=("Klavika Med Caps", 13,"bold"), foreground=corL, background=corPri).place(x=315, y=123)
   ttk.Label(gerenteja, text="Entrada:", font=("Klavika Med Caps", 13,"bold"), foreground=corL, background=corPri).place(x=315, y=163)
   ttk.Label(gerenteja, text="Email:", font=("Klavika Med Caps", 13,"bold"), foreground=corL, background=corPri).place(x=315, y=203)
   ttk.Label(gerenteja, text="Função:", font=("Klavika Med Caps", 13,"bold"), foreground=corL, background=corPri).place(x=315, y=243)
   ttk.Label(gerenteja, text="Exercida", font=("Arial", 8,"bold"), foreground=corL, background=corPri).place(x=321, y=261)
   

   
   ttk.Label(gerenteja, text="Nome:", font=("Klavika Med Caps", 13,"bold"), foreground=corL, background=corPri).place(x=30, y=123)
   ttk.Label(gerenteja, text="CPF:", font=("Klavika Med Caps", 13,"bold"), foreground=corL, background=corPri).place(x=30, y=163)
   ttk.Label(gerenteja, text="RG:", font=("Klavika Med Caps", 13,"bold"), foreground=corL,background=corPri).place(x=30, y=203)
   ttk.Label(gerenteja, text="Salário:", font=("Klavika Med Caps", 13,"bold"), foreground=corL, background=corPri).place(x=30, y=243)

   nome = ttk.Entry(gerenteja, font=("Arial", 12), width=20)
   nome.place(x=115, y=123)
   cpf = ttk.Entry(gerenteja, font=("Arial", 12), width=20)
   cpf.place(x=115, y=163)
   rg = ttk.Entry(gerenteja, font=("Arial", 12), width=20)
   rg.place(x=115, y=203)
   salario = ttk.Entry(gerenteja, font=("Arial", 12), width=20)
   salario.place(x=115, y=243)

   nasci = ttk.Entry(gerenteja, font=("Arial", 12), width=20)
   nasci.place(x=435, y=123)
   entra = ttk.Entry(gerenteja, font=("Arial", 12), width=20)
   entra.place(x=435, y=163)
   email = ttk.Entry(gerenteja, font=("Arial", 12), width=20)
   email.place(x=435, y=203)
   funcao = ttk.Entry(gerenteja, font=("Arial", 12), width=20)
   ...
   def salv():
      ...
      if c == f or (f == 1 and c == 0):
         if rep == 2:
            ...
            if nome.get() != "" and rg.get() != "" and cpf.get() != "" and salario.get() != "" and email.get() != "" and nasci.get() != "" and entra.get() != ""  and funcao.get() != "" and (variavelSC.get() == 1 or variavelCS.get() == 1 or variavelCA.get() == 1): 
               salvaDado(f)
               labr = ttk.Label(gerenteja, text="Pressione o botão denovo para fechar a janela", font=("Klavika Med Caps", 14), background="gray20", foreground="white", width=40)
               labr.place(x=109, y=391)
               gerenteja.after(3500, labr.destroy)
               rep += 1
      else:
         if c == 0:
            salvaDado(1)
   ...
   def prox():
      ...
      if c >= 0 and c < f and f != 1:
         ...
         if v == True and nome.get() != "" and rg.get() != "" and cpf.get() != "" and salario.get() != "" and email.get() != "" and nasci.get() != "" and entra != ""  and funcao != "" and (variavelSC.get() == 1 or variavelCS.get() == 1 or variavelCA.get() == 1):
            try:
               if c == 0:
                  lab.configure(text="Gerente %dº"%(c+2))
                  c += 2
               else:
                  lab.configure(text="Gerente %dº"%(c+1))
                  c += 1
               ...
               limpa()
               #######
               lab.configure(text="Gerente %dº"%(c))
      else:
         v = verificarUnica(f)
         if v == True and nome.get() != "" and rg.get() != "" and cpf.get() != "" and salario.get() != "" and email.get() != "" and nasci.get() != "" and entra.get() != ""  and funcao.get() != "" and (variavelSC.get() == 1 or variavelCS.get() == 1 or variavelCA.get() == 1):
            lab.configure(text="Gerente %dº"%(f))
            try:
               nm = listaV[f]
               #######
               limpa()
   ...
   def mostraCS():
      global gerenteja, senhaCS;
      senhaCS.configure(show="")
      def troca():
         senhaCS.configure(show="*")
      gerenteja.after(600, troca)
   def mostraCA():
      global gerenteja, senhaCA;
   ...
   btSC = Checkbutton(gerenteja, text="NÃO CRIAR CONTA?", takefocus=0, foreground=corL, background=corPri,\
                      font=("Klavika Med Caps", 11, "bold"), activeforeground=corL, variable=variavelSC, command=disableSC)
   btSC.place(x=30, y=290)
   btCS = Checkbutton(gerenteja, text="CRIAR CONTA APENAS DE GERENCIAR?", takefocus=0, foreground=corL, background=corPri,\
                      font=("Klavika Med Caps", 11, "bold"), activeforeground=corL, variable=variavelCS, command=disableCS)
   btCS.place(x=30, y=320)
   btCA = Checkbutton(gerenteja, text="CRIAR CONTA ADMINISTRADOR?", takefocus=0, foreground=corL, background=corPri,\
                      font=("Klavika Med Caps", 11, "bold"), activeforeground=corL, variable=variavelCA, command=disableCA)
   btCA.place(x=30, y=350)

   scs = ttk.Label(gerenteja, text="Senha:", font=("Klavika Med Caps", 13,"bold"), foreground=corL, background=corPri)
   scs.place(x=325, y=320)
   sca = ttk.Label(gerenteja, text="Senha:", font=("Klavika Med Caps", 13,"bold"), foreground=corL, background=corPri)
   sca.place(x=325, y=350)

   senhaCS = ttk.Entry(gerenteja, font=("Arial", 12), width=10, takefocus=0, show="*")
   senhaCS.place(x=390, y=320)
   senhaCA = ttk.Entry(gerenteja, font=("Arial", 12), width=10, takefocus=0, show="*")
   senhaCA.place(x=390, y=350)

   ima = PhotoImage(file="C:/ProgramData/TMS/Imagens/seca.png")
   buttCS = ttk.Button(gerenteja, image=ima, takefocus=0, command=mostraCS)
   buttCS.place(x=486, y=319)
   buttCA = ttk.Button(gerenteja, image=ima, takefocus=0, command=mostraCA)
   buttCA.place(x=486, y=349)
   
   #######
   cancela = Button(gerenteja,text="Cancelar", command=canc, relief=GROOVE, background=corPri,\
                    font=("Klavika Med Caps", 11, "bold"), width=11, foreground=corL, activeforeground=corL,\
                    takefocus=0)
   cancela.place(x=110, y=390)

   anterio = Button(gerenteja, text="<< Anterior", command=ante, relief=GROOVE,  background=corPri,\
                    font=("Klavika Med Caps", 11, "bold"), width=11, foreground=corL, activeforeground=corL,\
                    takefocus=0)
   anterio.place(x=220, y=390)

   proximo = Button(gerenteja, text="Proximo >>", command=prox, relief=GROOVE,  background=corPri,\
                    font=("Klavika Med Caps", 11, "bold"), width=11, foreground=corL, activeforeground=corL,\
                    takefocus=0)
   proximo.place(x=330, y=390)

   salva = Button(gerenteja, text="Salvar",  command=salv, relief=GROOVE,  background=corPri,\
                  font=("Klavika Med Caps", 11, "bold"), width=11, foreground=corL, activeforeground=corL,\
                  takefocus=0)
   salva.place(x=440, y=390)

   lab = ttk.Label(gerenteja, text="Gerente %dº"%(1), font=("Klavika Med Caps", 14,"bold"), foreground=corL, background=corPri)
   lab.place(x=290, y=80)

   instrucao = tix.Balloon(gerenteja, status=status, initwait=100, state='both')
   ###
   ...
   ###
   instrucao.bind_widget(logo, balloonmsg='todas as alterações feitas nos dados\ndos gerentes devem ser salvas\n(clicando no botão "salvar")',
   statusmsg='siga as instruções de funcionamento')
                         
   gerenteja.config(bg=corPri)
   gerenteja.geometry("%dx%d+%d+%d"%(xt/2.1, yt/1.7, xt/3.3, yt/5))
   gerenteja.overrideredirect(1)
   gerenteja.mainloop()
